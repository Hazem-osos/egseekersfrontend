...
export default function FreelancerConnectsPage() {
  ...
  const fetchConnectsData = async () => {
    try {
      ...
      const [connectsRes, packagesRes] = await Promise.all([
        axios.get("http://localhost:5001/api/connects"),
        axios.get("http://localhost:5001/api/connects/packages"),
      ]);
  ...
  const handlePurchaseConnects = async (packageId: string) => {
    try {
      ...
      await axios.post("http://localhost:5001/api/connects/purchase", {
        packageId,
        paymentMethod,
      });

      // Refresh connects data
      await fetchConnectsData();
    } catch (err) {
      console.error("Error purchasing connects:", err);
      setError("Failed to purchase connects. Please try again later.");
    } finally {
      setPurchasing(false);
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {packages.map((pkg) => (
              <Card key={pkg.id} className={selectedPackage === pkg.id ? "border-primary" : ""}>
                <CardHeader>
                  <CardTitle>{pkg.amount} Connects</CardTitle>
                ...
                <CardFooter>
                  <Button
                    className="w-full"
                    onClick={() => handlePurchaseConnects(pkg.id)}
                    disabled={purchasing}
                  >
