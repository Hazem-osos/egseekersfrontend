  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const proposalResponse = await axios.get(`https://egbackend-6zw2.onrender.com/api/proposals/${proposalId}`, {
          headers: { Authorization: `Bearer ${token}` }
        })
  ...
  const handleUpdateProposalStatus = async (status: 'ACCEPTED' | 'REJECTED') => {
    try {
      const response = await axios.patch(
        `https://egbackend-6zw2.onrender.com/api/proposals/${proposalId}`,
        { status },
        {
          headers: {
